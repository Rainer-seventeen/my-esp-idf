...
static const char * NVS_KEY = "sender";
...
/* 传入 addr 是为了将 lpn 地址存放到数组中 */
static void example_ble_mesh_parse_node_comp_data(const uint8_t *data, uint16_t length, const uint16_t addr)
{
    ...
    cid = COMP_DATA_2_OCTET(data, 0);   // 厂商标识符
    pid = COMP_DATA_2_OCTET(data, 2);   // 产品标识符
    vid = COMP_DATA_2_OCTET(data, 4);   // 版本标识符
    crpl = COMP_DATA_2_OCTET(data, 6);  // 最小重播保护列表条目数
    feat = COMP_DATA_2_OCTET(data, 8);  // 功能标识符
    ...
    case ESP_BLE_MESH_CFG_CLIENT_GET_STATE_EVT:
        if (param->params->opcode == ESP_BLE_MESH_MODEL_OP_COMPOSITION_DATA_GET) {
            ...
            example_ble_mesh_parse_node_comp_data(param->status_cb.comp_data_status.composition_data->data,
                param->status_cb.comp_data_status.composition_data->len, param->params->ctx.addr);
